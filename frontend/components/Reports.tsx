...
export default function Reports() {
  ...
  const fetchReportData = async (reportType: string) => {
    ...
    try {
      ...
      switch (reportType) {
        case 'sales':
          url = `http://localhost:5000/api/reports/daily-sales?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`;
          break;
        case 'waste':
          url = `http://localhost:5000/api/reports/food-waste?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`;
          break;
        case 'forecast':
          const days = Math.ceil((new Date(dateRange.endDate).getTime() - new Date(dateRange.startDate).getTime()) / (1000 * 60 * 60 * 24));
          url = `http://localhost:5000/api/reports/ingredient-forecast?days=${days}`;
          break;
      }
