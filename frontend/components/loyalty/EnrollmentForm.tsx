...
interface EnrollmentFormProps {
  onSubmit: (userId: string, initialPoints: number) => Promise<{ success: boolean; error?: string }>;
  onCancel: () => void;
...
const EnrollmentForm: React.FC<EnrollmentFormProps> = ({
  ...
  const fetchGuests = async () => {
    try {
      ...
      if (!hasToken) {
        setError('Please login as admin to access this feature');
        setLoading(false);
        return;
      }
      ...
      const enrolledUserIds = enrolledMembers.map(member => 
        typeof member.userId === 'object' && member.userId?._id 
          ? member.userId._id 
          : member.userId
      );
      ...
      const errorMessage = err.message || 'Failed to fetch guests';
      if (errorMessage.includes('Access denied') || errorMessage.includes('token')) {
        setError('Authentication required. Please login as admin to access this feature.');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setError(result.error || 'Failed to enroll guest');
    }
    setSubmitting(false);
  };
