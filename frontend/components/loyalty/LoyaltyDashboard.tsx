...
interface LoyaltyDashboardProps {
  ...
  loading?: boolean;
  onEnrollNew: () => void;
  onDeleteMember: (guestId: string) => void;
}
...
const LoyaltyDashboard: React.FC<LoyaltyDashboardProps> = ({
  ...
  const [filter, setFilter] = useState<string>('all'); // all, platinum, gold, silver, inactive
              ...
              <div 
                ...
                style={{
                  width: members.length > 0 
                    ? `${Math.min((members.reduce((sum, m) => sum + (m?.points || 0), 0) / members.length / 10000) * 100, 100)}%` 
                    : '0%'
                }}
              ></div>
            </div>
