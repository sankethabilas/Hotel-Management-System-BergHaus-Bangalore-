  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        action: {
          ...formData.action,
          points: formData.action.points ? parseFloat(formData.action.points.toString()) : undefined,
          multiplier: formData.action.multiplier ? parseFloat(formData.action.multiplier.toString()) : undefined
        },
    ...
    setFormData(prev => ({
      ...
      conditions: {
        ...
        tierRestrictions: prev.conditions.tierRestrictions.includes(tier)
          ? prev.conditions.tierRestrictions.filter((t: string) => t !== tier)
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-900">
                  {stats.executionStats?.reduce((sum: number, stat: any) => sum + stat.count, 0) || 0}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-orange-900">
                  {stats.executionStats?.find((s: any) => s._id === 'success')?.totalPoints.toLocaleString() || 0}
                </p>
              </div>
            </div>
          </div>
      ...
      {activeTab === 'top-rules' && stats?.topRules && (
        <div className="bg-white rounded-lg shadow">
          ...
          <div className="divide-y divide-gray-200">
            {stats.topRules.map((rule: any, index: number) => (
                ...
                {formData.action.type === 'award_points' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({
                        ...formData,
                        action: { ...formData.action, points: parseInt(e.target.value) || 0 }
                      })}
                      required
                      min="1"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                {formData.action.type === 'multiply_points' && (
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setFormData({
                        ...formData,
                        action: { ...formData.action, multiplier: parseFloat(e.target.value) || 1 }
                      })}
                      required
                      min="1"
                      step="0.1"
