...
interface FeedbackResponseProps {
  feedback: Feedback;
  mode?: 'view' | 'respond' | 'edit';
  onSubmit: (id: string, response: string) => Promise<{ success: boolean; error?: string }>;
  onCancel: () => void;
  loading?: boolean;
}

const FeedbackResponse: React.FC<FeedbackResponseProps> = ({
  feedback,
  mode = 'view',
  onSubmit,
  onCancel,
  ...
  const [charCount, setCharCount] = useState((feedback.adminResponse || '').length);
  const minLength = 10;
    ...
    <div className="bg-white rounded-lg shadow">
      ...
      <div className="p-6">
        ...
        <div className="bg-gray-50 p-4 rounded-md mb-6">
          ...
          <p className="mt-3 text-sm text-gray-700">{feedback.comments}</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div>
            ...
            {feedback.status === 'responded' && feedback.respondedAt && (
              <p className="mt-1 text-xs text-gray-500">
                {mode === 'view' ? 'Responded' : 'Last responded'} on {formatDate(feedback.respondedAt)}
              </p>
            )}
          </div>
