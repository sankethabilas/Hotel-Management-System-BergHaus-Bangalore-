...
interface FeedbackListProps {
  feedbacks: Feedback[];
  loading?: boolean;
  onViewFeedback: (feedback: Feedback, mode: 'view' | 'respond' | 'edit') => void;
...
const FeedbackList: React.FC<FeedbackListProps> = ({
  ...
  // Use useMemo to prevent re-calculation during render and avoid setState warnings
  const filteredFeedback = useMemo(() => {
    // Ensure feedbacks is an array
    const feedbackArray = Array.isArray(feedbacks) ? feedbacks : [];
    
    // Filter
    let filtered = feedbackArray.filter(feedback => {
      // Status filter
      if (filterStatus === 'pending' && feedback.status !== 'Pending' && feedback.status !== 'pending') return false;
      if (filterStatus === 'responded' && feedback.status !== 'Responded' && feedback.status !== 'responded' && !feedback.adminResponse) return false;
      ...
      if (searchTerm && !guestName.toLowerCase().includes(search) && !comments.toLowerCase().includes(search)) {
        return false;
      }
  ...
  }, [feedbacks, filterStatus, filterRating, sortBy, searchTerm]);
                ...
                <tr key={feedback._id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      feedback.status === 'Responded' || feedback.adminResponse 
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-3">
                      ...
                      {!feedback.adminResponse && (feedback.status === 'Pending' || !feedback.status) ? (
                        <button 
