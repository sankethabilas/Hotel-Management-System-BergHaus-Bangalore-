...
export default function GuestOrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      // In a real app, you'd filter by guest info, but for demo purposes, we'll fetch all orders
  ...
  const filteredOrders = orders.filter(order => {
    const matchesSearch = order.orderNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         order.customerInfo.roomNumber.toLowerCase().includes(searchTerm.toLowerCase());
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-gray-600">
                        <p>Room: {order.customerInfo.roomNumber} • {order.customerInfo.name}</p>
                        <p>Placed: {new Date(order.createdAt).toLocaleString()}</p>
                        <p>Payment: {order.paymentMethod} • Total: ${order.totalAmount.toFixed(2)}</p>
                      </div>
                    </div>
                        ...
                        <div className="flex-1">
                          ...
                          {item.customization && (
                            <div className="mt-2 p-2 bg-white rounded text-xs border">
                              ...
                              {item.customization.dietaryRestrictions.length > 0 && (
                                <div className="text-gray-600">• Dietary: {item.customization.dietaryRestrictions.join(', ')}</div>
                              )}
                              {item.customization.modifications.length > 0 && (
