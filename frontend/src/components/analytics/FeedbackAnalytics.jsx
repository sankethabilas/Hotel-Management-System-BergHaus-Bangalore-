  ...
  return <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <div className="flex items-center mt-1">
            ...
            <div className="ml-2 flex">
              {[...Array(5)].map((_, i) => <svg key={i} className={`h-5 w-5 ${i < Math.floor(summary.overallRating) ? 'text-yellow-400' : i < summary.overallRating ? 'text-yellow-200' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
          ...
          {trends.ratingChange !== 0 && (
            <p className={`mt-2 flex items-center text-sm ${trends.ratingChange > 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <span>{trends.ratingChange > 0 ? '+' : ''}{trends.ratingChange.toFixed(1)} since last period</span>
            </p>
          )}
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {trends.feedbackChange !== 0 && (
            <p className={`mt-2 flex items-center text-sm ${trends.feedbackChange > 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <span>{trends.feedbackChange > 0 ? '+' : ''}{Math.abs(trends.feedbackChange).toFixed(0)}% since last period</span>
            </p>
          )}
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {charts.ratingDistribution.some(item => item.value > 0) ? (
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={charts.ratingDistribution.filter(item => item.value > 0)} cx="50%" cy="50%" labelLine={false} outerRadius={100} fill="#8884d8" dataKey="value" label={({
                  name,
                  percent
                }) => `${name} (${(percent * 100).toFixed(0)}%)`}>
                    {charts.ratingDistribution.map((entry, index) => <Cell key={`cell-${index}`} fill={RATING_COLORS[index % RATING_COLORS.length]} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        {charts.categoryRatings.length > 0 ? (
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={charts.categoryRatings} layout="vertical" margin={{
              top: 5,
              right: 30,
              left: 20,
              bottom: 5
                ...
                <Bar dataKey="rating" fill="#d4af37" radius={[0, 4, 4, 0]}>
                  {charts.categoryRatings.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
