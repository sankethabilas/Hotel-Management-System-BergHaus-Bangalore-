...
export default function PromotionManagement() {
  ...
  const fetchPromotions = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5001/api/admin/promotions', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingPromotion 
        ? `http://localhost:5001/api/admin/promotions/${editingPromotion._id}`
        : 'http://localhost:5001/api/admin/promotions';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`http://localhost:5001/api/admin/promotions/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
