...
import { getItems, updateItem, deleteItem } from "../../../../lib/inventoryApi";
...
export default function InventoryItemsPage() {
  ...
  const handleIncrement = async (id: string, field: keyof InventoryItem) => {
    const item = items.find(i => i._id === id);
    if (item) {
      await updateItem(id, { [field]: (item[field] as number) + 1 });
      fetchItems();
    }
  };

  const handleDecrement = async (id: string, field: keyof InventoryItem) => {
    const item = items.find(i => i._id === id);
    if (item && (item[field] as number) > 0) {
      await updateItem(id, { [field]: (item[field] as number) - 1 });
      fetchItems();
    }
  };
            ...
            <tr key={item._id} className="text-center">
              <td className="border p-1 sm:p-2 flex items-center space-x-2 sm:space-x-3 min-w-[200px]">
                ...
                <div className="flex flex-col space-y-1">
                  <span className="font-medium text-xs sm:text-base">
                    <Link href={`/admin/inventory/items/${item._id}`} className="text-blue-600 hover:underline cursor-pointer">
                      {item.name}
                    </Link>
                  </span>
              ...
              <td className="border p-1 sm:p-2">
                <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0 sm:space-x-2">
                  <button onClick={() => setEditItem(item)} className="px-2 py-1 bg-green-500 text-white rounded text-xs sm:text-sm hover:bg-green-600">Edit</button>
                  <button onClick={() => handleDelete(item._id)} className="px-2 py-1 bg-red-500 text-white rounded text-xs sm:text-sm hover:bg-red-600">Delete</button>
                </div>
              </td>
            </tr>
          ))}
