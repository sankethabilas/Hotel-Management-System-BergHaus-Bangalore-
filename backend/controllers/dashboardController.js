...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const avgRatingChange = avgRatingPrevious > 0
      ? ((avgRatingCurrent - avgRatingPrevious) || 0).toFixed(1)
      : avgRatingCurrent > 0 ? (avgRatingCurrent || 0).toFixed(1) : '0';
    ...
    const satisfactionData = satisfactionTrend.map(item => ({
      ...
      satisfaction: parseFloat((item.satisfaction || 0).toFixed(1))
    }));
    ...
    const response = {
      stats: {
        ...
        averageRating: {
          value: (avgRatingCurrent || 0).toFixed(1),
